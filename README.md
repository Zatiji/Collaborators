...
- **Lists and entries**: local-only for now. They live in on-device SQLite ([`src/db/schema.ts`](src/db/schema.ts), [`src/db/queries.ts`](src/db/queries.ts)) as plain rows — the `ops`-log/CRDT data layer described in the spec hasn't been built yet, so nothing syncs across devices yet.
